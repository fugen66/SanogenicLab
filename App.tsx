...
const App: React.FC = () => {
  ...
  const handleRun = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Ошибка при общении с ИИ. Проверьте API_KEY в настройках Vercel.');
    }
  };
      ...
      <header className="py-6 px-8 max-w-6xl mx-auto w-full flex flex-col md:flex-row justify-between items-center gap-6 border-b border-slate-800/50">
        <div className="flex items-center gap-4">
          ...
          <div className="text-left">
            <h1 className="text-2xl font-black tracking-tighter text-white uppercase">Sanogenic Lab</h1>
      ...
      <main className="max-w-5xl mx-auto w-full px-6 py-12">
          ...
          <div className="animate-in fade-in slide-in-from-bottom-8 duration-1000 space-y-12">
            <div className="text-center space-y-4">
              <h2 className="text-4xl md:text-7xl font-black text-white tracking-tighter max-w-4xl mx-auto leading-tight">
                {activeTab === AppTab.ANALYSIS && 'Очистите разум от обид'}
                {activeTab === AppTab.JOURNAL && 'Поймите свои эмоции'}
                {activeTab === AppTab.METAPHOR && 'Притча для осознания'}
              </h2>
            </div>

            <form onSubmit={handleRun} className="bg-slate-900/30 border border-white/5 p-10 rounded-[3.5rem] shadow-3xl space-y-10 backdrop-blur-xl text-left">
              <div className="space-y-4">
                <label className="text-xs font-black text-teal-500 uppercase tracking-widest block">
                ...
                {activeTab === AppTab.ANALYSIS && (
                  <textarea value={thoughtInput} onChange={(e) => setThoughtInput(e.target.value)} placeholder="Я чувствую обиду, потому что..." className="w-full bg-slate-950/40 border border-slate-800 rounded-[2rem] p-8 text-2xl text-white outline-none focus:ring-2 focus:ring-teal-500 transition-all min-h-[200px]" />
                )}
                ...
                {activeTab === AppTab.JOURNAL && (
                  <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      ...
                      <input value={context} onChange={(e) => setContext(e.target.value)} placeholder="Причина?" className="w-full bg-slate-950/40 border border-slate-800 rounded-2xl p-6 text-white outline-none" />
                    </div>
                  </div>
                )}

                {activeTab === AppTab.METAPHOR && (
                  <textarea value={metaphorInput} onChange={(e) => setMetaphorInput(e.target.value)} placeholder="Опишите проблему..." className="w-full bg-slate-950/40 border border-slate-800 rounded-[2rem] p-8 text-2xl text-white outline-none focus:ring-2 focus:ring-teal-500 transition-all min-h-[200px]" />
                )}
              </div>

              <button type="submit" className="w-full py-8 bg-white text-slate-950 font-black rounded-[2rem] hover:scale-[1.01] transition-all flex items-center justify-center gap-4 uppercase tracking-widest text-sm shadow-2xl">
          ...
          <div className="animate-in fade-in zoom-in-95 duration-700 space-y-10 text-left">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="md:col-span-2 space-y-8">
                  <div className="bg-slate-900 border border-slate-800 p-12 rounded-[3rem] shadow-2xl">
                    <h4 className="text-[10px] font-black text-teal-500 uppercase tracking-widest mb-8">Анализ механизма</h4>
                  ...
                  <div className="bg-gradient-to-br from-teal-600 to-teal-800 text-white p-12 rounded-[3rem] shadow-2xl">
                    <h4 className="text-[10px] font-black uppercase tracking-widest mb-4 opacity-70">Саногенный Щит</h4>
            ...
            {emotionResult && (
              <div className="bg-slate-900 border border-slate-800 p-12 rounded-[3rem] shadow-2xl max-w-3xl mx-auto">
                ...
                <p className="text-2xl text-slate-300 italic font-serif mb-8">"{emotionResult.reflection}"</p>
                <div className="p-8 bg-slate-950 rounded-2xl border border-teal-500/20">
                   <h5 className="text-[10px] font-black text-teal-500 uppercase mb-2">Совет по угашению:</h5>
            ...
            {metaphorResult && (
              <div className="bg-slate-900 border border-slate-800 p-12 rounded-[3rem] shadow-2xl max-w-2xl mx-auto text-center">
                <h4 className="text-3xl font-black text-white mb-6 uppercase tracking-tighter">{metaphorResult.title}</h4>
                <div className="text-lg text-slate-300 font-serif leading-relaxed italic mb-8 whitespace-pre-line">
                  {metaphorResult.story}
                </div>
                <p className="font-bold text-teal-500 text-xl tracking-tight">«{metaphorResult.moral}»</p>
              </div>
            )}
          </div>
        )}
        ...
        {appState === AppState.ERROR && (
          <div className="text-center py-20 bg-slate-900/50 border border-red-500/20 rounded-[3rem] p-10 max-w-2xl mx-auto">
             ...
             <div className="text-white mb-6 font-bold text-xl">{error}</div>
             <button onClick={reset} className="px-10 py-4 bg-teal-600 hover:bg-teal-500 text-white rounded-2xl font-bold uppercase text-xs transition-all shadow-lg">Попробовать снова</button>
          </div>
        )}
      </main>
